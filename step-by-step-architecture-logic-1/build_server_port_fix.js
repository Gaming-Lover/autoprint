...
let HTTPS_PORT = parseInt(process.env.HTTPS_PORT || process.env.PORT || 3000, 10);
let HTTP_PORT = parseInt(process.env.HTTP_PORT || 3080, 10);
...
function startServers() {
  ...
  httpsServer.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
      console.log('⚠️  Port ' + HTTPS_PORT + ' is already in use.');
      HTTPS_PORT++;
      console.log('🔄 Trying next available port: ' + HTTPS_PORT + '...');
      setTimeout(() => {
...
console.log('server.js port fix applied');
