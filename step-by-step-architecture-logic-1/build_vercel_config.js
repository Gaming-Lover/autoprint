...
// 2. Create api/index.js (Vercel Serverless Function Handler)
const apiIndexCode = `const url = require('url');
const path = require('path');
const fs = require('fs');
...
module.exports = async function handler(req, res) {
  ...
  if (pathname.endsWith('/status') || pathname === '/api/status') {
  ...
  if (pathname.endsWith('/setup') || pathname === '/api/setup') {
  ...
  if (pathname.endsWith('/login') || pathname === '/api/auth/login') {
    const body = await parseJsonBody(req);
  ...
  if (pathname.endsWith('/dashboard') || pathname === '/api/dashboard') {
  ...
  if (pathname.endsWith('/settings') || pathname === '/api/settings') {
    if (!checkAdminAuth(req)) return sendJson(res, 401, { success: false, message: 'Unauthorized' });
  ...
  if (pathname.endsWith('/upload') || pathname === '/api/upload') {
    try {
  ...
  if (pathname.endsWith('/create-order') || pathname === '/api/create-order') {
    const body = await parseJsonBody(req);
  ...
  if (pathname.endsWith('/verify-payment') || pathname === '/api/verify-payment') {
    const body = await parseJsonBody(req);
...
fs.writeFileSync('api/index.js', apiIndexCode, 'utf8');
console.log('vercel.json and api/index.js written successfully');
